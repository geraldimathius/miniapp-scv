      ...
      <main className="flex-1 max-w-[1500px] w-full mx-auto p-4 sm:p-6 md:p-10 space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 text-xs sm:text-sm mt-1">
              Setujui pendaftaran anggota baru, kelola hak akses role, atau hapus akun pengguna.
            </p>
          </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex flex-wrap items-center gap-1.5 w-full sm:w-auto">
            ...
            >
              Disetujui ({users.filter(u => u.status === 'approved').length})
            </button>
            <button
              type="button"
                      ...
                      <tr key={u.id} className="hover:bg-slate-50/60 transition-colors">
                        ...
                        <td className="py-3.5 px-4">
                          <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold ${
                            ...
                            {u.status === 'approved' ? '✓ Disetujui' : u.status === 'pending' ? '⏳ Menunggu Approval' : '✕ Ditolak'}
                          </span>
                        </td>
                        ...
                        <td className="py-3.5 px-4 text-right">
                          {isLoading ? (
                            <span className="text-[11px] text-slate-400 font-semibold">Memproses...</span>
                          ) : (
                            <div className="inline-flex items-center gap-1.5 justify-end">
                              ...
                              {u.status === 'approved' && !isSelf && (
                                ...
                                >
                                  {u.role === 'master' ? 'Jadikan Member' : '👑 Jadikan Master'}
                                </button>
                              )}
